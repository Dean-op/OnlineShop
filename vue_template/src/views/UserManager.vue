          ...
          <el-card shadow="hover" class="stat-card">
            <div class="stat-content">
              <div class="stat-left">
                ...
                <div class="stat-title">商家用户</div>
              </div>
              <div class="stat-value">{{ userStats.farmers }}</div>
            </div>
          </el-card>
        </el-col>
        <el-col :span="6">
      ...
      <el-table :data="users" v-loading="listLoading" border style="width: 100%">
        ...
        <el-table-column prop="role" label="角色" width="120">
          <template slot-scope="scope">
            ...
            <el-tag v-else-if="scope.row.role === 'FARMER'" type="success">商家</el-tag>
  ...
  methods: {
        ...
        if (res.code === '0') {
          ...
          this.users = res.data.records.filter(user => {
            ...
            // 管理员可以看到普通用户和商家
              ...
              return ['USER', 'FARMER'].includes(user.role)
            }
            return false
          })
          // 使用后端返回的总数，而不是过滤后的长度
    ...
    calculateUserStats() {
      this.userStats = {
        ...
        farmers: this.users.filter(user => user.role === 'FARMER').length,
