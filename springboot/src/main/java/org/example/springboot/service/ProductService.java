...
import org.example.springboot.controller.AiController;
...
import org.example.springboot.util.*;
...
public class ProductService {
    ...
    public String getAiResponse(String question) throws Exception {
        // 1. 限制查询商品数量（避免过多商品导致内容超长）
        QueryWrapper<Product> queryWrapper = new QueryWrapper<>();
        queryWrapper.last("LIMIT 50"); // 只取前50个商品
        List<Product> products = productMapper.selectList(queryWrapper);

        // 构建系统提示词
        StringBuilder systemPrompt = new StringBuilder();
        systemPrompt.append("你是「文旅电商平台」的智能客服助手，专注于为用户提供文旅特色商品推荐、");
        systemPrompt.append("景点文化介绍、购物咨询等服务。请用友好、专业的语气回答用户问题。\n\n");

        if (products.isEmpty()) {
            // 无商品时直接提问
            return AiUtils.getAiResult(systemPrompt + "用户的问题是：" + question);
        }

        // 2. 精简商品信息（只保留关键字段，减少字符数）
        StringBuilder information = new StringBuilder(systemPrompt);
        information.append("以下是平台商品信息（最多50个）：\n");
        for (Product product : products) {
            // 只保留名称、价格、库存三个核心字段
            information.append("商品名称：").append(product.getName())
                    .append("，价格：").append(product.getPrice())
                    .append("，库存：").append(product.getStock()).append("\n");

            // 3. 二次保险：如果中途长度接近限制，提前停止拼接
            if (information.length() > 400000) { // 预留10万字符给用户问题
                information.append("...（商品信息已精简，仅展示部分商品）\n");
                break;
            }
        }

        // 4. 拼接用户问题（控制总长度）
        information.append("请根据以上商品信息回答用户的问题：").append(question);

        // 5. 最终长度检查（避免极端情况）
        if (information.length() > 500000) {
            // 超过最大限制时，直接返回提示（或进一步截断）
            return "商品信息过多，无法处理当前问题，请尝试更具体的提问。";
        }

        return AiUtils.getAiResult(information.toString());
    }
