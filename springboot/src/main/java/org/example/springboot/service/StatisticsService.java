...
public class StatisticsService {
    ...
    public Map<String, Object> getMonthlyOrderStatistics(Long farmerId) {
        ...
        // 统计本月订单数
        long currentMonthOrders = getOrderCount(currentMonth, farmerId);
        // 统计上月订单数
        long lastMonthOrders = getOrderCount(lastMonth, farmerId);
    ...
    public Map<String, Object> getMonthlySalesStatistics(Long farmerId) {
        Map<String, Object> result = new HashMap<>();
        ...
        // 统计本月销售额
        double currentMonthSales = getSalesAmount(currentMonth, farmerId);
        // 统计上月销售额
        double lastMonthSales = getSalesAmount(lastMonth, farmerId);
    ...
    private long getOrderCount(YearMonth yearMonth, Long farmerId) {
        ...
        if (farmerId != null) {
            ...
            wrapper.inSql(Order::getProductId, 
                "SELECT id FROM product WHERE farmer_id = " + farmerId);
        }
        
        return orderMapper.selectCount(wrapper);
    }

    private double getSalesAmount(YearMonth yearMonth, Long farmerId) {
        LambdaQueryWrapper<Order> wrapper = new LambdaQueryWrapper<>();
        wrapper.ge(Order::getCreatedAt, yearMonth.atDay(1).atStartOfDay())
               .lt(Order::getCreatedAt, yearMonth.plusMonths(1).atDay(1).atStartOfDay())
