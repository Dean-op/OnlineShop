...
public class OrderController {
    ...
    public Result<?> getOrdersByPage(
            ...
            @RequestParam(required = false) Integer platformIntervention,
            @RequestParam(required = false) Long farmerId,
            @RequestParam(defaultValue = "1") Integer currentPage,
            @RequestParam(defaultValue = "10") Integer size) {
        return orderService.getOrdersByPage(userId, id, status, platformIntervention, farmerId, currentPage, size);
    }
    ...
    @Operation(summary = "用户申请平台介入")
    @PostMapping("/{id}/platform-intervention")
    public Result<?> applyPlatformIntervention(@PathVariable Long id, @RequestParam String reason) {
        return orderService.applyPlatformIntervention(id, reason);
    }

    @Operation(summary = "平台处理退款介入")
    @PutMapping("/{id}/platform-intervention")
    public Result<?> handlePlatformIntervention(
            @PathVariable Long id,
            @RequestParam Long handlerId,
            @RequestParam Integer refundStatus,
            @RequestParam(required = false) String remark) {
        return orderService.handlePlatformIntervention(id, handlerId, refundStatus, remark);
    }
